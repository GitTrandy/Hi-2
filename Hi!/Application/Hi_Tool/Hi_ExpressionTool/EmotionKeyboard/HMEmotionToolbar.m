...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self setupButton:@"发送" tag:HMEmotionTypeLxh];
...
- (UIButton *)setupButton:(NSString *)title tag:(HMEmotionType)tag
{
    ...
    if (count == 1) { // 第一个按钮
        ...
        [button setBackgroundImage:[UIImage resizedImage:@"compose_emotion_table_send_normal-1"] forState:UIControlStateNormal];
        [button setBackgroundImage:[UIImage resizedImage:@"compose_emotion_table_send_normal-1"] forState:UIControlStateNormal];
